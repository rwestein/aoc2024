...
class WalkOffMapException(Exception):
    pass
...
class Dec6:
    ...
    def step_towards(self, newx, newy):
        if newx < 0 or newx >= self.sizex or newy < 0 or newy >= self.sizey:
            raise WalkOffMapException('Out of bounds')
        self.posx, self.posy = newx, newy
    ...
    def mark_current_as_visited_with_direction(self):
        if self.matrix[self.posy][self.posx] == '.':
            self.matrix[self.posy][self.posx] = self.direction

    def make_move(self, mark_path):
        newx, newy = self.calculate_new_position()
        if newx < 0 or newx >= self.sizex or newy < 0 or newy >= self.sizey:
            # Walk off the map
            raise WalkOffMapException()
        if self.matrix[newy][newx] in '#O':
            self.rotate_guard()
        else:
            self.step_towards(newx, newy)
            if mark_path == 'special':
                self.mark_current_as_visited_with_direction()
            elif mark_path:
    ...
    def try_scenario_with_obstacle(self, x, y, mark_path=False):
        if self.visit_matrix[y][x] != 'X':
            return False

        obstacle_creates_loop = False
        self.restore_matrix()
        orig_content = self.matrix[y][x]
        self.matrix[y][x] = 'O'
        if self.walk(mark_path=mark_path):
            obstacle_creates_loop = True
        # Restore
        self.matrix[y][x] = orig_content
        return obstacle_creates_loop


    def analyze_two_positions(self, input):
        self.matrix = self.parse_input(input)
        self.determine_position()
        self.walk(mark_path=True)
        ...
        x, y = 97, 41
        loop = self.try_scenario_with_obstacle(x, y)
        print(x, y, loop)

        x, y = 97, 24
        loop = self.try_scenario_with_obstacle(x, y, mark_path=True)
        print(x, y, loop)
        self.print()
    ...
    def solve2(self, input, print_output=False):
        ...
        for y in range(self.sizey):
            for x in range(self.sizex):
                if self.try_scenario_with_obstacle(x, y):
